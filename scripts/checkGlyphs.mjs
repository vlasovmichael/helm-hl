...
//    · знак валюты не приписывают руками к форматтеру, который его уже ставит.
//
//  🚨 Такие переводы доделываются заходами: каждый раз находится ещё одно
//  место. Пусть находит `npm run guards`, а не пользователь.
// ─────────────────────────────────────────────────────────────────────
...
for (const file of walk(ROOT)) {
  ...
  src.split("\n").forEach((line, i) => {
    ...
    // 🚨 fmtPrice/fmtUsd/fmtMoney/fmtNotional уже возвращают строку со знаком
    // валюты и с «—» для пустого значения. Дописанный рядом `$` давал в
    // интерфейсе «$$7.2965»; ловим шаблон, а не конкретное место.
    const FMT = "(?:fmtPrice|fmtUsd|fmtMoney|fmtMoneyAbs|fmtNotional)";
    const doubleSign = new RegExp(`\\$\\$\\{\\s*${FMT}\\s*\\(|["'\`]\\$["'\`]\\s*\\+\\s*${FMT}\\s*\\(`);
    if (doubleSign.test(line)) {
      problems.push(`${rel}:${i + 1}  лишний $ перед форматтером — он уже ставит знак  ${line.trim().slice(0, 60)}`);
    }
