...
// 🚨 ГЛАВНОЕ ПРО ЛИМИТ HL. Лимит /info — НЕ 1200 запросов в минуту, а 1200
// ЕДИНИЦ ВЕСА в минуту на IP. Вес зависит от типа запроса (см. WEIGHTS ниже):
// большинство весят 20, «лёгкие» (allMids/l2Book/clearinghouseState/…) — 2,
// userRole — 60. То есть тяжёлых можно ~60/мин = ОДИН В СЕКУНДУ. Всё, что
// считает ШТУКИ, а не вес, ловит 429 сотнями в сутки. Ниже — бюджет по весу со
// скользящим окном 60с; concurrency/min-gap оставлены против мгновенных залпов.
...
// 🚨 Без этого новоприбывшие расхватывают освободившийся вес мимо ждущих, и
// отдельные запросы стоят минутами при половинной загрузке.
