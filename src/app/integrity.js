 ...
 * 🚨 Одной сверки монеты и стороны мало: DB-строка тогда живёт со СТАРЫМ
 * entry_price, трейл считает % от чужого входа и пишет фиктивный PnL.
 *
...
// Исход проверки переживает вызов и попадает в шапку дашборда
// (см. core/healthRegistry.js).
//
...
//
// Один такой проход — норма, API отстаёт регулярно. Затяжной — уже поломка,
// которую нечем заметить. Поэтому лаг считается подряд: разовый warn,
// затяжной fail.
...
export async function integrityCheck() {
  ...
  try {
    ...
    //
    // 🚨 Занятая маржа сама по себе лагом НЕ является: осиротевшие reduce-only
    // ордера закрытой позы держат её тоже, и круг замыкается — ордера не
    // снимаются, пока строка открыта, а строка не закрывается, пока маржа
    // выглядит занятой. Поэтому сначала спрашиваем ордера: reduce-only без
    // позиции — сирота, а не лаг индексатора.
    if (
...
export async function orphanCheck() {
  ...
  // молча уходил в hands-off без стопа. Монета, которой
  ...
  if (config.trading.adoptEnabled) {
    try {
      ...
      // и диагностировался только лазаньем в БД.
