...
//
// Ручная позиция подхватывается в СВОБОДНЫЙ слот как strategy_id='adopt', бот
// сразу ставит реальный reduce-only стоп на бирже. Бот ведёт ВЫХОД, не вход.
//
...
// Последнее решение adopt по монете: coin → краткая причина-строка, чтобы
// «почему не усыновил» было видно на дашборде, а не только в логах.
// Усыновлённая монета причину чистит.
...
// 🚨 Возраст входа из fills приходит с лагом ~30с, и в это окно openTime=null.
// Один только null сваливает в кучу «свежак, fill не проиндексирован» и
// «реально старый orphan» — и поза 30с висит без стопа.
// Различаем по тому, ВИДЕЛ ли бот рождение позы: absent→present при живом боте
// → заведомо свежая, усыновляем сразу. Висела уже на старте → возможен старый
// orphan, остаёмся осторожны.
...
export function decideAdoptByAge({ openTime, firstSeen, now, maxAgeMs }) {
  ...
  // 3. Поза висела уже на старте или first-seen неизвестен → возможен старый
  // orphan → консервативно НЕ трогаем.
...
// Сирота-алерт: поза уже ГЛУБЖЕ планового стопа на момент усыновления. 🚨 Стоп
// тут ставить нельзя — reduce-only триггер уже in-the-money, это мгновенный
// market-close по худшей цене. Вместо него: не усыновляем + громкий пуш,
// «резать или держать» решает оператор. Алерт повторяем не чаще раза в 30м.
const _orphanAlertAt = new Map(); // coinUpper → ts последнего алерта
const ORPHAN_REALERT_MS = 30 * 60_000;

// Алерт «стоп не встал на бирже». 🚨 Ошибка биржи при постановке SL — всегда
// риск-алерт, не строка в логах: протухший API-ключ иначе на дни оставляет
// ручные позы без стопа молча.
 ...
 *
 * Ручные входы приходят с биржи без снапшота, и без этого из журнала не
 * вытащить ни ось «фейдил или следовал тренду», ни час входа. Досниаем то, что
 ...
 * поэтому adopt-строки сравнимы с бот-стратегиями.
 *
 ...
 *
 * Позиция на бирже = знаковая сумма fills (и бот, и ручных). Идём по fills во
 * времени, держим net-размер; открытие текущей позы = момент, когда |net| ушёл
 * от нуля или сменил знак. 🚨 Фильтровать бот-fills по oid нельзя: тогда
 * бот-закрытие усыновлённой позы не обнуляет net и копятся фантомные ноги —
 * свежая поза читается как многодневная и отказывает по too-old, оставаясь без
 * стопа.
 *
 * 🚨 ОКНО FILLS: fetchUserFills отдаёт ~60д, и позиция, ПЕРЕСЕКАЮЩАЯ границу
 * окна, стартует с ложного net=0 — смещение, которое никогда не обнуляется, и
 * каждый выход во флэт читается как открытая поза. Поэтому якоримся к истине с
 * биржи: offset = replayEndNet − currentNet вычитается из всей траектории.
 * currentNet null → offset 0.
 *
 * @param {object}  args
...
export function resolveManualOpenTime({ coin, fills, currentNet = null }) {
  ...
  // 🐛 ЛАГ ИНДЕКСАТОРА: лента заканчивается ФЛЭТОМ, а биржа
  // показывает позицию — значит открывающего fill'а в ленте просто ещё нет
  // (HL индексирует за 10-30с, adopt смотрит чаще). Якорь тут применять НЕЛЬЗЯ:
  // он задуман для усечения окна СЛЕВА (поза открылась раньше окна), а здесь
  // данных не хватает СПРАВА. Сдвиг всей траектории превращает каждый реальный
  // проход через ноль в смену знака, и openTime уползает на последний из них.
  // ARB: оператор вошёл в 12:50:15, бот записал 11:45:14 — время закрытия ПРЕДЫДУЩЕЙ
  // позы, возраст 1:10 вместо шести минут. Честный ответ — «не знаю»: выше по
  // стеку это значит «подхватить по first-seen», а не «выдумать время».
  if (currentNet != null && Math.abs(currentNet) > EPS && Math.abs(net) < EPS) {
    return null;
  }
...
export async function maybeAdoptManualPosition(manualPositions) {
  ...
  for (const ex of list) {
    ...
    // которую бот только что торговал, не усыновлялась никогда). Убрано.
    ...
    // и fills молчат → возможен старый orphan → не трогаем.
    // ex.szi — знаковый размер позиции с биржи: якорит replay fills, чтобы
    // усечение 60д-окна не давало ложный возраст.
    ...
    // Триггеры няньки — в bot_oid_log. 🚨 Классификация round-trip'ов идёт по
    // oid (userFills.isBotFill): без этой записи fill по стопу или цели читается
    // как ручной, и выход няньки в леджере выглядит как «закрыл я».
    ...
    // депо — это всегда серии входов в минусе.
