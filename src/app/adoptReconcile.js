...
// 🚨 Пока fills лагают (~30с), null от них означает и «свежак», и «старый orphan».
// Различаем по тому, видел ли бот рождение позы: absent→present при живом боте =
// свежая; висела уже на старте = осторожничаем.
...
/**
 * Снапшот рыночного контекста в момент усыновления ручного входа: без него из
 * журнала не вытащить ни ось «фейдил или следовал тренду», ни час входа. Берём
 * из тех же ring-буферов, что наполняет scout (цена 4ч, OI ~20мин) — семантика
 * колонок совпадает с Hunter'ом, строки сравнимы.
 *
 * Всё nullable: пустой буфер усыновление не блокирует.
 *
 * @param {string} coin
 ...
 * Идём по fills во времени, держим net-размер; открытие = момент, когда |net|
 * ушёл от нуля или сменил знак.
 *
 * 🚨 Фильтровать бот-fills по oid нельзя: бот-закрытие тогда не обнуляет net,
 * копятся фантомные ноги, свежая поза читается как многодневная.
 * 🚨 Позиция, пересекающая границу 60д-окна, стартует с ложного net=0 — якоримся
 * к бирже: offset = replayEndNet − currentNet вычитается из всей траектории.
 *
 * @param {object}  args
...
export function resolveManualOpenTime({ coin, fills, currentNet = null }) {
  ...
  // 🚨 Лента кончилась флэтом, а на бирже поза = открывающего филла ещё нет.
  // Якорь тут не применять: он усекает окно СЛЕВА, а данных не хватает СПРАВА —
  // сдвиг траектории превратит каждый проход через ноль в смену знака. Честный
  // ответ «не знаю»; время входа выдумывать нельзя.
...
/**
 * Время открытия текущей ОТКРЫТОЙ ручной позиции (unix ms) или null — не смогли
 * определить, тогда не подхватываем. `currentNet` якорит replay к бирже.
 */
 ...
 * reduce-only стоп. `manualPositions` уже очищен от монет, которыми бот владеет.
 *
...
export async function maybeAdoptManualPosition(manualPositions) {
  ...
  const now = Date.now();
  // 🚨 First-seen ДО выхода на пустом списке: иначе ушедшие монеты копятся в карте,
  // а цикл с 0 поз не метит boot.
  ...
  for (const ex of list) {
    ...
    // 🚨 Hunter cross-cooldown здесь не применяется: оператор вошёл руками сам,
    // няньке остаётся повесить стоп. С гардом такие позы оставались без стопа.

    // Гард возраста: fills авторитетны; их ещё нет, но поза родилась при живом
    // боте → свежак; висела на старте и fills молчат → возможен orphan, не трогаем.
    // ex.szi якорит replay, чтобы усечение окна не дало ложный возраст.
 ...
 * когда HL отдаёт open-fill, лента классифицирует позу как 'adopted' по точному
 * entry-матчу, а не как 'manual'. Дёшево — fills уже в кэше. Не пришёл за
 * BACKFILL_GIVEUP_MS — бросаем.
 */
