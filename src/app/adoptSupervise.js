...
export async function superviseAdoptPositions(priceFn = getLivePrice) {
  ...
  for (const pos of positions) {
    ...
    // Правило работает и на позах, открытых до старта процесса: пик после
    // рестарта восстанавливается из свечей за первый же тик (adoptPeakTruth),
    // то есть известен. 🚨 Без такого восстановления взводиться «задним числом»
    // на неизвестном пике нельзя — это снять лимитку и тут же выйти по рынку.
