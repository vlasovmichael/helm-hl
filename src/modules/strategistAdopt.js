...
// ⛔ ВЫКЛЮЧЕН (ADOPT_TRAIL_ENABLED=false): фактическая отдача от пика была
// ~40% при пороге 30%. Выход теперь либо BE-храповик, либо биржевой SL,
// либо рука, а звонок о развороте шлёт peak-alert.
...
// Осмотр позиции редок, и порог отдачи бывает перепрыгнут уже в момент решения
// (видим 38% при пороге 30%). Эти две карты пишут в лог, что было на ПРЕДЫДУЩЕМ
// осмотре и сколько времени прошло. Торговлю не меняют.
...
// Восстановление мягкого состояния с диска после рестарта (см. шапку
// adoptTrailStore.js). Ключи в JSON — строки, мапы здесь по числовому
// position.id, отсюда Number.
 ...
 *
 * Тиковый пик считается по allMids и пропускает проколы между кадрами, из-за
 * чего трейл не взводится. Берём максимум двух источников: понижать пик нельзя
 * никогда (MFE не убывает по определению), повышать — ровно ради этого.
 *
