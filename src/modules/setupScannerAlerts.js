...
// Делегат в core fireNtfy (тихий час + priority + журнал колокольчика + почта
// считаются там). Свой топик NTFY_TOPIC_SWING. Локальный isQuietHour оставлен
// (тесты), но боевое решение по priority принимает core.
async function fireNtfy(title, message, tags) {
  await fireNtfyCore({
    topic: process.env.NTFY_TOPIC_SWING || config.ntfy.topic,
    title,
    message,
    tags,
  });
