...
export async function runOnce(now = Date.now(), fetchCandles = getFifteenMinCandles) {
  ...
  // ── Гейт режима: горячий рынок (BTC трендит). Холодно → ни одного пуша ──
  let btcER = null;
  if (FADEHOT_REGIME_GATE) {
    ...
    btcER = regime.btcER;
    if (!regime.hot) {
      logger.info(`[FadeHotAlerts] рынок холодный (BTC ER ${btcER == null ? 'n/a' : btcER.toFixed(2)} < ${FADEHOT_BTC_ER_MIN}, breadth ${breadth}) — фейдов нет`);
      return;
    }
  }
  ...
  for (const item of candidates.slice(0, MAX_FETCH)) {
    ...
    logger.info(`[FadeHotAlerts] 🎯 FADE ${v.side} #${item.coin} | ход30м ${moveStr}, ER ${v.er.toFixed(2)}, BTC ER ${btcER == null ? 'n/a' : btcER.toFixed(2)}`);
    await fireNtfy({
      topic: TOPIC,
      ...
      message:
        ...
        `режим: BTC ER ${btcER == null ? 'n/a' : btcER.toFixed(2)} · breadth ${breadth} (рынок горячий)\n` +
...
export function startFadeHotAlerts() {
  ...
  timer.unref?.();
  logger.info(`[FadeHotAlerts] started — every ${INTERVAL_MS / 60_000}min, cooldown ${COOLDOWN_MS / 60_000}min, topic=${TOPIC || '—'}`);
}
