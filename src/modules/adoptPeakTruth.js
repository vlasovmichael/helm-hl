...
// ─────────────────────────────────────────────────
// 🚨 Пик (MFE) по одним allMids врёт: кадров ~22/мин, быстрый прокол между ними
// в пик не попадает — а лимитка на бирже такое движение ловит.
//
// Low/high минутных свечей проколы видят: свечу рисует биржа по сделкам.
// Поэтому пик = МАКСИМУМ двух источников, тикового и свечного. Максимум, а не
// замена: тик свежее (свеча догоняет до минуты), свеча полнее.
//
// ⚠️ Цена этого: пик растёт → растёт и откат от пика, на котором считается
// give-back трейла. Фитиль в один принт может взвести трейл и тут же закрыть
// позу «по откату» от цены, по которой реально было не выйти.
//
