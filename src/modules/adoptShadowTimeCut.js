...
// ─────────────────────────────────────────────────
// Мотивация (2026-07-02, разбор 131 adopt-сделки PROD): 23 стопа съели ~$70 при
// общем PnL +$47, средний стоп −3.5% против победителя +1.1%. Ключевое: НИ ОДИН
// стопнутый не видел MFE ≥1% (max 0.99%) — это не «выход упустил», это «вход не
// сработал, поза умирала часами» (SOL 220м, HYPE 496м под водой). Снижать BE-arm
// бесполезно (спасает 2-3 сделки), а time-cut попадает в ~9 из 23.
//
// Модель: поза старше TIMECUT_MIN минут ни разу не показала MFE ≥ GREEN_PCT →
// would-be выход по текущей цене. Честно просимулировать нельзя (свечей по тикам
// нет) — поэтому МЕРИМ в проде, НЕ трогая реальные выходы (паттерн
// hunterShadowExits). Строка в shadow_exits на close: td_* = time-cut модель,
// ch_* = actual (вторая модель не используется — «доехала до реального выхода»).
