...
// свою монету выделенной, пока торгует руками.
    ...
    for (const p of manualPositions)
      if (p?.coin) {
        ...
        posMap.set(p.coin, {
          ...
          // у бот-сделки. Для неусыновлённых ручных позиций bot=null.
          bot: p.bot ?? null,
          adopted: !!p.adopted,
          source: "manual",
        });
...
// провоцировать оператора лезть руками в сделку, от которой он ушёл.
...
export function hmPosHintInner(coin) {
  ...
  // «Жив ли движ» читается по momentum-ячейкам самой строки.
  const chips = [];
  ...
  const chipsHtml = chips
    .map(([k, t, title]) => {
      const tip = title ? ` data-card="${escapeHtml(title)}"` : "";
