...
/** Пустая оболочка `.modal` — для диалогов, которых нет в статической разметке. */
export function shell(id, { wide = false } = {}) {
  ...
  el.innerHTML = `
    ...
    <div class="modal__panel${wide ? " modal__panel--wide" : ""}" role="dialog" aria-modal="true"></div>`;
