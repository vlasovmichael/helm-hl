...
export function open(root, { onClose } = {}) {
  ...
  root.hidden = false;
  // Появление и уход ведёт ЯДРО, а не фича. До 04.09.2026 анимацию имел один
  // trade ticket (свой класс is-open + свои таймеры), остальные диалоги просто
  // возникали и пропадали — при том, что открываются они из одних и тех же
  // мест и рядом друг с другом.
  //
  // 🚨 Между `hidden = false` и классом обязателен принудительный reflow:
  // браузеру нужно зафиксировать НАЧАЛЬНОЕ состояние (opacity 0 + сдвиг),
  // иначе переход не с чего начинать. requestAnimationFrame здесь не годится —
  // в неактивной вкладке кадры не выдаются, и callback не вызовется никогда:
  // панель осталась бы невидимой (ловили ровно это на trade ticket).
...
/**
 * Закрывает диалог: снимает is-open, ждёт уход панели и только потом прячет.
 * Фокус и замок прокрутки возвращаются СРАЗУ — они про управление, а не про
 * картинку, и ждать анимации им незачем.
 */
export function close(root) {
  ...
  // Прячем после ухода панели. Страховка по таймеру обязательна: transitionend
  // не придёт, если диалог закрыли во вкладке без кадров или анимация выключена
  // системной настройкой «меньше движения».
  const hide = () => {
...
/**
 * ЯДРО: одна оболочка, разное содержимое.
 *
 * Диалог — это всегда шапка (знак + заголовок + крестик), тело и, если есть,
 * ряд действий. Меняется только тело; всё остальное — оболочка, и собирать её
 * заново в каждой фиче незачем: именно так пять диалогов и разъехались.
 *
 * Фича, которой нужна своя вёрстка целиком (trade ticket — лист кошелька со
 * своим заголовком по центру), по-прежнему может звать shell/open/close
 * напрямую: ядро не запрещает, а избавляет от повторов.
 *
 * @param {object} o
...
export function show({
  ...
  // Содержимое кладём в свой контейнер, а не в панель: иначе перерисовка тела
  // сносила бы разметку, которую фича положила рядом (шапку листа, подложку).
