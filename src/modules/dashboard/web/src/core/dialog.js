...
//
// Escape, замок прокрутки, фокус-трап и возврат фокуса живут ЗДЕСЬ, а не в
// каждой модалке. Две ловушки, из-за которых так:
// • замок прокрутки СЧИТАЮЩИЙ, а не булев — иначе закрытие второго из двух
// открытых диалогов снимает замок, пока первый ещё открыт;
// • Escape общий — своя подписка ловит только «свой» диалог.
//
 ...
 *
 * 🚨 На верхнем уровне модуля его быть не может: юнит-тесты импортируют
 * tradeTicket.js в Node, где `document` не существует, и файл падает на
 * импорте.
 */
let keysBound = false;
function bindKeys() {
