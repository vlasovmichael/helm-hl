...
function reconcileRows(tbody, items) {
  ...
  // Во ФОНОВОЙ вкладке не анимируем вовсе. Браузер не выдаёт ей кадры, и
  // анимация повисает на нулевом кадре — строка осталась бы смещённой на весь
  // dy до возвращения на вкладку. Ровно этот баг уже ловили с
  // requestAnimationFrame; с WAAPI он выглядит так же, только тише.
  if (reduceMotion || document.visibilityState !== "visible") return;

  // ── LAST + INVERT + PLAY ──
  //
  // Строка едет на новое место, а не перескакивает: перестановка — главное
  // событие этой таблицы, и без проезда список просто «моргает» другим
  // порядком.
  //
  // 🚨 Анимация идёт через Web Animations API, а НЕ через style.transition.
  // Тик приходит каждые 2с, а до него на каждой строке успевает смениться весь
  // innerHTML (цены живые). С transition это давало рывки: новый переход
  // навешивался поверх незакончившегося, браузер брал за старт уже сдвинутое
  // состояние, и строка дёргалась. `el.animate()` отменяет предыдущий проезд
  // явно (cancel), считает от текущего положения и живёт на композиторе —
  // перерисовка содержимого его не трогает. 04.09.2026, после трёх заходов на
  // «плавно».
  for (const { el, key, entering } of ordered) {
    ...
    el.__hmMove = el.animate(
      ...
      {
        // 460 мс с плоским выходом: строка приходит на место и замирает, без
        // отскока (пружину пробовали — перелёт читается как дрожь).
        duration: 460,
