...
function reconcileRows(tbody, items) {
  ...
  for (const { el, key, entering } of ordered) {
    ...
    // Одна длительность и одна кривая на любую перестановку.
    //
    // 🚨 04.09.2026 здесь была «умная» анимация: длительность по корню из
    // пройденного пути + пружина (--spring). Обе идеи оказались хуже простой:
    // пружина даёт ПЕРЕЛЁТ, и строка в таблице живых цен слегка отскакивает,
    // проезжая мимо своего места, — на восьми строках сразу это читается как
    // дрожь; разная длительность у соседних строк ломает ощущение, что список
    // переставился ОДНИМ движением. Вернулись к прежнему: 420 мс, чистый
    // ease-out, без отскока.
