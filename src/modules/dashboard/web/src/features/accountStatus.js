...
// Фон #sec-position тем зеленее (краснее), чем лучше день: Today (realized) +
// Σ uPnL открытых монет. Кривая sqrt делает мелкие суммы видимее, MAX_MIX держит
// текст читаемым. 🚨 Только эта секция и нигде больше.
...
// ── Производные метрики ручной (HANDS-OFF/ADOPTED) позиции. Один источник и для
// сборки HTML, и для патча-на-месте, иначе цифры разъезжаются.
...
// патч на месте меняет текст, не пересобирая карточку.
...
export function renderHeader(status) {
  ...
  // Unified-аккаунт: equity = spot.total + perp.uPnL, available = spot.total −
  // spot.hold. Разбивки perp/spot нет: это один пул.
...
// Читает core/healthRegistry.summary() из статус-кадра: состояние фидов должно
// быть видно без похода в docker logs.
//
...
function renderDailyBadge() {
  ...
  // Комиссии — пожиратель №1 минуса. Цена оборота перед глазами: сегодня в
  // бейдже, неделя — в tooltip.
  const feeStr =
...
// 🚨 Таймер — чекпоинт «перечитать график», а не «ход кончился»: «ровно 3 свечи
// 15m» в данных не подтвердилось (разворот 52–58%).
const FLOOR_TIMER_SEC = 15 * 60;
const DAY_SEC = 24 * 3600;
// 🚨 Санити-потолок щедрый: отсекает мусорную дату входа, но не живую позу.
// С сутками счётчик исчезал у всего, что висит дольше дня.
...
export function renderManualPositions(list) {
  ...
  const blocks = list
    .map((p) => {
      ...
      // ADOPTED = на позе висит стоп+трейл няньки. Не подхвачена — показываем
