...
// ─────────────────────────────────────────────────
// Гипотеза `entry-into-continuation` (реестр, 02.09.2026): вход В СТОРОНУ уже
// случившегося сильного движения даёт результат хуже, чем вход против него.
// Разбор 650 ручных сделок 25.05–02.09.2026:
//   · по тренду 15м −0.177 на сделку против −0.053 против тренда;
//   · тренд 1ч < −5% → −0.487 (t=−3.4, единственный срез, переживший Бонферрони);
//   · лонги −38.77 против шортов +22.89.
//
// ⚠️ Карточка НИЧЕГО не предсказывает и не выбирает монету. Она отвечает ровно
// на один вопрос: «эта сторона по этой монете сейчас — вход по движению или
// против него». Гипотеза post-hoc, найдена на тех же данных, поэтому до 60
// новых сделок вердикт — это метка для форвард-замера, а не рекомендация.
//
