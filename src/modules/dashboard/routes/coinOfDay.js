...
import { getCodPicks, getHistorySince } from '../../../core/database.js';
...
/**
 * Цена BTC из того же снапшота рынка, что кормит скан. Нужна форвард-логу как
 * база бенчмарка: без неё нельзя будет отделить отработку фейда от общего хода
 * рынка. Отдельный запрос не делаем — BTC и так в снапшоте.
 */
function btcPriceFromMarket(rows) {
  const row = (rows || []).find((r) => String(r.coin || '').toUpperCase() === 'BTC');
...
async function build(now) {
  ...
  try {
    logScanPicks(scan, btcPriceFromMarket(state.latestHunter), now);
  } catch (err) {
