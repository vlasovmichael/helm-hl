...
// ─────────────────────────────────────────────────
// Список адресов заморожен (docs/winners-preregistration.json),
...
// позиции. Форвардный вердикт считает только tools/winners.mjs track по
// снимкам лидерборда — увиденное здесь на него не влияет.
//
// 🚨 LOW-приоритет ждёт бюджета не дольше 1.5с и штатно отваливается с
// WeightBudgetTimeoutError: косметика не имеет права копить очередь и тянуть
// за собой тик. Поэтому адреса опрашиваются по одному, а не залпом, и
// последний удачный ответ по каждому показывается с пометкой возраста —
// «данные 3 мин назад» честнее, чем «не ответил».
...
// tools/winners.mjs track по снимкам лидерборда, дата решения.
