...
// ── 🚨 Переписано 26.08.2026: этот файл убивал бота ──────────────────────────
// Прежняя реализация на КАЖДЫЙ запрос читала два месячных файла целиком
// (28МБ + 25МБ одной utf8-строкой), делала text.split("\n") — ещё столько же
// копией — и парсила все 3572 снимка по 232 монеты: ~830 тысяч мелких объектов
// {oi,f,px,v} в куче. Замер зонда (allocProbe) на живом проде:
//
//   [Alloc] http:GET /api/oi-collector/overview ×1 +175.6МБ (max 175.6МБ)
//
// Поверх рабочей базы бота в ~190МБ это и есть те залпы +183/+200МБ за 30
// секунд, от которых процесс умирал с FATAL heap limit (см. память
// container_oom_silent_restart_2026_08_02: четыре смерти, интервал между ними
// был лотереей — 1д 8ч, потом 37 минут). Лотерея объяснилась: автообновления у
// страницы нет, залп прилетал ровно тогда, когда оператор открывал вкладку OI.
//
// Здесь три правила, которые нельзя откатывать «для простоты»:
