...
export function handlePretrade(req, res) {
  ...
  const ranked = Object.entries(coins)
    .map(([name, c]) => ({
      coin: name, vlm: c.vlm, atr: c.atr, cost: costOf(c, mode),
      share: shareOf(c, mode), gap: gapOf(pick(c.grid, mode), mode),
    }))
  ...
  const feeBudgetUsd = day && equity > 0 && config.trading.dailyFeeBudgetPct > 0
    ...
    : null;
  const costUsd = notional ? (notional * costOf(c, mode)) / 1e4 : null;
  ...
  res.json({
    ...
    coin: {
      ...
      atr: c.atr, half: c.half,
      cost: costOf(c, mode), share: shareOf(c, mode),
