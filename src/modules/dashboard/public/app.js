...
const HELP_CONTENT = {
  ...
  coinglass: {
    title: "Coinglass — деривативная карта рынка",
    lead: "coinglass.com агрегирует данные по фьючерсам со всех бирж (Binance, Bybit, OKX, HL и др.): открытый интерес (OI), funding, long/short ratio, ликвидации. Для fader-скальпера это <b>контекст</b>: где толпа, где её стопы, кто платит за позицию.",
    sections: [
      {
        title: "С чего начать каждый день (3 минуты)",
        sub: "Когда открываешь Coinglass — пройди эти 3 экрана в таком порядке, прежде чем смотреть Hot Movers:",
        rows: [
          ["1. <b>BTC Liquidation Heatmap</b>", "Главная страница → Liquidation Heatmap. Показывает где висят кластеры стопов. Если ярко-жёлтое пятно выше текущей цены — туда могут «слить» (long squeeze)."],
          ["2. <b>Long/Short Ratio</b>", "Futures → Long/Short Ratio. >60% лонгов = толпа в лонгах, контрарианский сигнал на SHORT-fade пампа."],
          ["3. <b>Funding Rate</b>", "Futures → Funding Rate. Положительный высокий funding (>0.05% / 8h) = шортов перебор оплачивают лонгов = риск squeeze вниз."],
        ],
      },
      {
        title: "Главные секции (меню сверху)",
        rows: [
          ["<b>Liquidations</b>", "Поток ликвидаций по биржам в реальном времени. Большие красные свечи = массовые лонг-ликвидации (цена резко вниз) — часто <b>дно локального дампа</b>, момент для LONG-fade."],
          ["<b>Liquidation Heatmap</b>", "Топ-фича. Цветные «облака» = плотность стоп-кластеров. Цена магнитится к ярким зонам. <b>Используй как target/risk-зоны</b>, не как сигнал входа."],
          ["<b>Open Interest</b>", "Сколько денег в открытых позициях. Резкий рост OI без роста цены = накопление позы (часто перед сильным движением)."],
          ["<b>Long/Short Ratio</b>", "По биржам и аккаунтам. Главное: <b>Top Trader Ratio</b> (киты) отличается от <b>Global Account</b> (розница). Розница в лонгах + киты в шортах = жди дамп."],
          ["<b>Funding Rate</b>", "Кто кому платит за удержание позы. Persistent положительный = быкам дорого = повод фейдить пампы; отрицательный = шортам дорого = повод фейдить дампы."],
          ["<b>Coin-by-coin (поиск)</b>", "Сверху ищи тикер (например NIL) — получишь funding/OI/L-S именно по этой монете на HL и других биржах."],
        ],
      },
      {
        title: "Конкретный workflow для Fader",
        sub: "Когда видишь спайк на Hot Movers и думаешь зайти — открой Coinglass и проверь:",
        rows: [
          ["A. Liquidation Heatmap", "Есть ли крупный кластер стопов <b>ПРОТИВ</b> твоего входа близко? Если да — НЕ заходи, цена пробьёт твой entry и сходит туда."],
          ["B. Funding на HL", "Если фейдишь памп — funding должен быть положительным (лонгов перебор). Если уже отрицательный — толпа уже в шортах, фейдить опасно."],
          ["C. OI delta за час", "OI вырос +5% за час во время пампа = реальное позиционирование, не MM-фейк. Фейдить осторожнее."],
          ["D. Recent liquidations", "Только что прошёл всплеск лонг-ликвидаций? Значит slabые лонги уже выбиты — следующая нога вниз менее вероятна."],
        ],
      },
      {
        title: "Чего НЕ делать",
        rows: [
          ["Не торговать «по картинке»", "Coinglass — контекст, не сигнал. Никогда не входи только потому что funding высокий."],
          ["Не верить retail L/S", "Global Long/Short Ratio часто шумный. Top Trader Ratio полезнее."],
          ["Не смотреть OI без контекста цены", "OI растёт = можно вверх и вниз. Считай Δ OI vs Δ price."],
        ],
      },
      {
        title: "Бесплатно vs Premium",
        sub: "Бесплатной версии хватает для манульного скальпинга: heatmap, OI, funding, L/S, ликвидации — всё доступно. Premium даёт исторические данные глубже 7 дней и алёрты — на старте не нужно.",
      },
    ],
  },
  aggrTrade: {
    title: "aggr.trade — multi-exchange tape",
    lead: "aggr.trade — это <b>лента сделок</b>, агрегированная со всех крупных бирж сразу. Показывает реальный поток ордеров: кто бьёт в маркет, на каких уровнях, как меняется баланс покупок/продаж. Для fader'а это «датчик микро-структуры»: видно момент, когда импульс выдыхается.",
    sections: [
      {
        title: "Что вообще на экране",
        rows: [
          ["<b>Chart</b>", "Свечи с агрегированной ценой (медиана/средняя с бирж). Линии — крупные принты."],
          ["<b>Trades feed</b>", "Лента сделок справа. Зелёные = market buy (агрессор покупатель), красные = market sell (агрессор продавец). Размер строки = размер сделки в USD."],
          ["<b>Bottom histogram</b>", "Гистограмма delta volume: зелёные бары = больше market-buy чем sell за интервал; красные = наоборот."],
          ["<b>CVD line</b>", "Cumulative Volume Delta — накопленная разница buy minus sell. Если CVD растёт, а цена нет = скрытое накопление; если падает, а цена нет = распределение."],
        ],
      },
      {
        title: "Как настроить под скальпинг (один раз)",
        rows: [
          ["1. Выбрать монету", "Сверху слева — поле для тикера. Введи BTCUSDT / ETHUSDT / NIL / любой. Pairs подтянутся со всех подключённых бирж."],
          ["2. Подключить биржи", "Settings → Exchanges → включи Binance, Bybit, OKX, Hyperliquid (через perp pair). Чем больше бирж — тем «жирнее» лента."],
          ["3. Поставить фильтр", "Settings → Trades → minimum size (например $5,000 или $10k). Иначе лента превратится в кашу. На крупных монетах ставь $25-50k."],
          ["4. Audio (опционально)", "Можно включить звук на крупные принты (>$100k). На NIL/мелких — отключи, оглохнешь."],
        ],
      },
      {
        title: "Что искать перед fade-входом",
        sub: "Открой aggr на той монете, что собираешься фейдить. Смотри 30-60 секунд:",
        rows: [
          ["<b>Замедление принтов</b>", "Был непрерывный поток зелёных market-buy → стал прерывистым → пошли первые красные. <b>Это твой entry-сигнал для SHORT-fade.</b>"],
          ["<b>Дивергенция CVD</b>", "Цена сделала новый хай, а CVD НЕ обновил хай. Значит покупки слабее. Классика для fade."],
          ["<b>Крупный принт «против»</b>", "Памп шёл, и вдруг один большой красный (например $200k market sell) — кто-то крупный начал распродажу. Хороший триггер."],
          ["<b>Absorption</b>", "Цена долбится в уровень, идёт волна market-sell, но цена НЕ падает — кто-то покупает «снизу». Это значит fade SHORT не сработает, сворачивай идею."],
        ],
      },
      {
        title: "Конкретный workflow для Fader",
        sub: "Hot Movers показал FADE-сетап → открой Coinglass (контекст) → открой aggr (триггер):",
        rows: [
          ["1.", "На Hot Movers увидел 🟢 FADE SHORT на условном #NIL после пампа."],
          ["2.", "Coinglass: funding NIL положительный, нет крупного кластера стопов выше → ок, входим в идею."],
          ["3.", "Открыл aggr.trade с тикером NIL, фильтр $1-2k (мелкая монета)."],
          ["4.", "Смотришь ленту 30 сек: ещё идут зелёные принты — <b>жди</b>."],
          ["5.", "Поток зелёных замедлился → пошли красные → CVD загнулся вниз → <b>вход SHORT.</b>"],
          ["6.", "После входа продолжай смотреть aggr: если опять пошёл сплошной зелёный поток — закрывай руками, не жди SL."],
        ],
      },
      {
        title: "Полезные настройки UI",
        rows: [
          ["Layout", "Можно разделить экран: слева chart, справа trades + delta histogram. Сохрани workspace через Settings."],
          ["Timeframe", "Для скальпинга 1m или tick chart. CVD считается по выбранному TF."],
          ["Theme", "Dark по умолчанию. Settings → Theme если хочется иначе."],
          ["Запомнить ссылку", "Твой URL `aggr.trade/cej9` — это shared workspace с пресетом. Сохрани в bookmarks."],
        ],
      },
      {
        title: "Чего НЕ делать",
        rows: [
          ["Не торговать только по aggr", "Лента показывает <b>что уже произошло</b>. Используй вместе с Coinglass-контекстом и Hot Movers-setup'ом."],
          ["Не реагировать на каждый принт", "На малых TF будет много шума. Жди подтверждения 2-3 события подряд."],
          ["Не забывать про spread", "На мелких монетах spread может съесть твой профит. Aggr показывает execution price, не bid/ask."],
        ],
      },
    ],
  },
