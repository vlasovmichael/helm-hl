...
// ─────────────────────────────────────────────────
//
// 🚨 Пик трейла и взвод BE-храповика в одной памяти не выживают: рестарт под
// открытой позой обнуляет peak, бот ведёт сделку заново, и плюсовая позиция
// спокойно уезжает в минус до биржевого стопа. Жёсткий SL это переживает (он
// resting-ордером лежит на бирже), весь мягкий выход — нет.
//
// Формат и приёмы те же, что в hunterCooldownStore: своя версия схемы,
// атомарная запись (tmp + rename), ленивая загрузка, отбрасывание орфанов по
// hard-TTL (позиция закрылась, пока бот лежал → clearAdoptState не вызвался).
//
// Пишем только PROD-позиции: у бумажных двойников состояние восстанавливать
// незачем.
