...
// появилась или исчезла. То же, что витрина на /lab, но по таймеру.
//
// ⛔ Наблюдение, а не участие: вердикт предзаявленного теста считает только
// tools/winners.mjs track по снимкам лидерборда. Пуши отсюда в расчёт не входят.
//
// СОБЫТИЕ здесь — только переходы flat→открыто и открыто→flat плюс разворот
// стороны. Доборы и частичные фиксации молчат: по филлам это шум (сотни в
// сутки против единиц смен состояния).
//
// 🚨 Это НЕ сигнал на вход: данные из СОСТОЯНИЯ счёта, между их сделкой и пушем
// проходит до интервала опроса, и цена в пуше — их вход, а не твой.
//
// Вес ≈ 18/мин при LOW-доле бюджета в 700. Приоритет LOW, торговый путь всегда
// впереди в очереди.
