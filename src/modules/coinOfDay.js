...
// Монеты, забаненные по журналу: системно сливали.
...
export function analyzeCoin({ coin, price, oiUsd, fundingRate, volume24hUsd, c1h, c15 }) {
  ...
  //
  // 🚨 Ни край диапазона, ни ход за сутки НЕ гейты, только баллы и сортировка.
  // Гейт заставляет сетап стирать самого себя по мере отработки: цена уходит от
  // хая → ход и позиция в диапазоне падают ниже порога → монета исчезает с
  // экрана посреди живого движения. Скор должен просаживаться плавно.
...
export function buildHeldView({ coin, analysis, position, pick, price }) {
  ...
  if (pick && Number.isFinite(pick.stop) && Number.isFinite(pick.target)) {
    // 🚨 R от ТВОЕГО входа, не от входа карточки: риск несёшь ты, от своей цены
    // до стопа плана. Иначе панель показывает R чужой сделки.
...
export async function scanCoinOfDay(marketRows, now = Date.now(), opts = {}) {
  ...
  //
  // 🚨 Ход за сутки и здесь НЕ фильтр, а ключ сортировки: с порогом монета с
  // отработавшим фейдом выпадает из пула ещё до скоринга. Слоты кандидатов
  // ограничены, поэтому топ забирают самые сходившие — по убыванию, не по порогу.
  const pool = rows
    .filter((r) => {
