...
// ─────────────────────────────────────────────────
//
// 🚨 trades.db теряет историю при каждой порче, после rebuild'а в ней остаётся
// только хвост. Источник правды за весь период — on-chain fills из
// userFillsByTime; модуль восстанавливает помесячную картину из них и от
// trades.db не зависит.
//
...
//
// closedPnl от HL — ДО комиссий (price PnL), fee идёт отдельной строкой.
// Net = Σ closedPnl − Σ fees + funding.
//
// Заморозка прошлых месяцев: userFillsByTime отдаёт ~60 дней, поэтому
// завершённые месяцы фиксируются в data/ledger_months.json по правилу
// «макс. число сделок выигрывает» — усохшее окно не перезатирает снапшот.
