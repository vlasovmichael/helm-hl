...
//
// 🚨 Кэш без вытеснения убивает процесс по heap limit V8 — раньше, чем rss
// упрётся в лимит контейнера (Node выводит heap_size_limit из cgroup и ставит
// его НИЖЕ). TTL решает только «идти в сеть или отдать кэш», записи он не
// удаляет; скаут и movers ходят по широкой вселенной (~1011 монет), и набор
// задетых монет расползается сутками — пара мегабайт в час до упора.
//
...
// Зачем отдельный кэш с самым коротким TTL: пик по WS-мидам (~22 кадра/мин)
// пропускает быстрые проколы, и трейл не взводится там, где цена реально была.
// Low/high 1m-свечи такие проколы видят.
