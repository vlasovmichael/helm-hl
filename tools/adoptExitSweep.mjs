...
// ─────────────────────────────────────────────────
// Вопрос оператора (15.08): настройки трейла не пересматривались с начала проекта,
// «мне кажется, я только теряю с этим трейлом». Вопрос законный, но опасный:
// перебрать сетку на своих же 500 сделках и объявить лучшую — это гарантированно
...
// против сравнения абсолютных доходностей, поэтому мощности хватает там, где
// на «есть ли эдж» её не было бы (см. память journal_decomposition_2026_07_16).
//
...
//     10 000 ресэмплов, 95% ДИ на РАЗНИЦУ. Абсолютные числа без ДИ не считаются
//     результатом (память feedback_stop_exit_edge_mantra).
...
//    выигрывают незаслуженно — реальный перелёт съедает ~0.05$ на сделку
//    (замер AdoptTrailProbe). Держать в уме при чтении близких результатов.
//  • Данные adopt — с 12.06.2026, это ~2 месяца, а не полгода.
//
// Запуск:
