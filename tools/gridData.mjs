...
// Глубина на 17.08.2026 (замерено, а не из документации): лимит 5000 баров —
// на ИНТЕРВАЛ, а не на историю, поэтому чем крупнее бар, тем длиннее окно.
//   15m → 52 дня · 1h → 208 дней · 4h → 833 дня (2.3 года) · 1d → 6 лет
// 4h — единственный интервал, на котором сегодня доступно НЕСКОЛЬКО режимов
// рынка, то есть где статус выше «кандидата» вообще достижим.
const IV_MS = {
