...
// ─────────────────────────────────────────────────
// ⚠️ ИЗМЕРЕНИЕ БАЗРЕЙТА, а не стратегия. Вопрос один: возвращается ли цена к
// пробитому структурному уровню. Что делать после возврата — здесь не спрашивается.
//
// 🚨 Процент без нулевой модели ничего не значит: цена задевает произвольные
// уровни постоянно, тем чаще, чем выше волатильность. Поэтому три модели, у
// каждой сломано ровно одно свойство:
// real — пробой структурного уровня, ждём возврата к нему.
// time — тот же коин и расстояние d, момент СЛУЧАЙНЫЙ. Ломает структуру.
// mirror — тот же бар и расстояние d, уровень с ДРУГОЙ стороны цены. Ломает
// направление, сохраняя момент, коин и волатильность. Самая жёсткая:
// real ≈ mirror значит, что «ретест» — переименованная амплитуда.
//
// 🚨 САМОПРОВЕРКА НА GBM обязательна. У real есть механическое преимущество:
// BOS случается, когда цена только что пересекла уровень, и он в шаге позади.
// Такой зазор real−null возникает и на случайном блуждании — прибор меряет его
// на синтетике, и на реальных данных зазор обязан эту цену артефакта превысить.
//
...
// BOS вверх close пробивает ближайший НЕПРОБИТЫЙ swing high выше цены
// ретест в следующие N=24 бара какой-нибудь low ≤ уровня
// вниз зеркально; уровень стреляет один раз
//
// Правило решения, объявленное заранее:
// 1. 80% опровергнуто, если 95% ДИ реального базрейта его не накрывает.
// 2. «Структура есть» — только если (real − mirror) превышает артефакт с GBM
// и ДИ этой разницы не задевает ноль.
// 3. 🚨 ДИ считается ПО МОНЕТАМ, а не по событиям: события внутри монеты не
// независимы, и биномиальный ДИ по пулу соврёт в разы.
//
// ⚠️ Данные: ~40 дней, один режим рынка — вывод к ним и привязан.
//
