...
//
// ── Главный источник ЛОЖНЫХ находок: замерший фид ──────────────────────────
...
// является. Отсюда STALE_MS: обе стороны должны быть свежими, иначе окно
// не засчитывается вообще. Пропуски пишем в stderr, чтобы их было видно.
//
...
function evaluate(coin) {
  ...
  if (now - hl.t > STALE_MS || now - bn.t > STALE_MS) {
    // Замерший фид: закрываем всё открытое как недостоверное, не эмитим.
