...
//
// ── ИСПРАВЛЕНО 14.08 (второй заход): выход подрезал сам себя ───────────────
// 12 сделок, из них 1 плюсовая — и семь закрылись с причиной «прибыльно», но
// в минус. Противоречие вскрыло дырку: выход срабатывал в момент, когда позиция
// впервые показывала ЛЮБОЙ плюс, а ордер на закрытие летит те же 220 мс, и
// тонкий плюс за это время исчезает. Плюсовые систематически подрезались,
// минусовые досиживали до таймаута: средняя жизнь плюсовой 26 с, минусовой 174 с.
// Правило само производило картинку «одна из двенадцати».
//
// Теперь выход требует ЗАПАС: прибыль должна превышать то, что задержка может
// съесть. Запас берём равным половине суммы спредов — ровно на столько цена и
// может уехать, пока ордер летит. Это не подгонка под результат: тот же самый
// член уже стоит в пороге входа, по той же причине.
//
// ── Фильтр по спреду: пустой стакан — не рынок ─────────────────────────────
// Девять из тех двенадцати сделок бот сделал по ACE, где спред Kraken был
// 120 бп. Книгу шириной 1.2% нельзя торговать: пересечь её дороже, чем весь
// бюджет издержек. Правило объявлено ЗАРАНЕЕ и привязано к смыслу, а не к
// результату: если спред площадки больше суммы комиссий, она не котирует
// торгуемый рынок, и «расхождение» там — шум внутри чужого спреда.
//
...
export class PaperBot {
  constructor(opts = {}) {
    ...
    // Шире этого спред площадки = не торгуемый рынок. По умолчанию — сумма
    // комиссий круга: пересекать книгу дороже, чем весь бюджет издержек, смысла нет.
    this.maxSpreadBp = Number(opts.maxSpreadBp
      ?? 2 * ((opts.takers?.hl ?? 4.5) + (opts.takers?.kr ?? 5)));
    ...
    this.stats = {
      ...
      skippedNoRoom: 0, skippedTooSmall: 0, skippedWideBook: 0,
    };
  }
  ...
  onTick(coin, pair, state, costBp) {
    ...
    // Пустой стакан — не рынок. Если спред площадки шире, чем весь бюджет
    // комиссий, её котировки нельзя торговать ни при каком расхождении.
    const spreadA = (a.ask - a.bid) / mid * 10_000;
    const spreadB = (b.ask - b.bid) / mid * 10_000;
    if (spreadA > this.maxSpreadBp || spreadB > this.maxSpreadBp) {
      this.stats.skippedWideBook++;
      return;
    }
  ...
  #maybeExit(now, key, trade, state) {
    ...
    // Запас: прибыль должна пережить полёт ордера. Половина суммы спредов —
    // ровно то, на что цена может уехать за 220 мс на этих книгах.
    const midNow = (a.bid + a.ask + b.bid + b.ask) / 4;
    const bufferUsd = trade.notional
      * ((a.ask - a.bid) + (b.ask - b.bid)) / midNow / 2;
    const profitable = markToMarket > bufferUsd;
    ...
    this.pending.push({
      ...
      sentAt: now, fillAt: now + this.latencyMs, reason: timedOut ? "timeout" : "target",
    });
  }
