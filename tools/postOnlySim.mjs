...
// ─────────────────────────────────────────────────
// Недостающее звено к tools/bookCollector.mjs (11.08). Коллектор копит стакан
// именно ради этого замера; feeAudit честно назвал свою цифру ПОТОЛКОМ и
// отказался считать экономию. Здесь считается не потолок, а разложение.
//
// ── Вопрос ─────────────────────────────────────────────────────────────────
// 100% наших входов — тейкер (1440 из 1440, feeAudit). Тейкер на HL платит
// 4.5 бп, мейкер 1.5 бп. Плюс post-only входит по пассивной стороне, то есть
// экономит ещё и спред. Итого арифметический выигрыш ≈ спред + 3 бп.
// При среднем результате сделки +7 бп это не косметика.
//
...
//    этого списка и раньше 11.08 в замер не попадают — они считаются отдельной
//    строкой «не покрыто», а не молча выбрасываются.
