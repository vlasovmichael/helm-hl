...
// копить снимки самим (2026-08-03).
//
// Это НЕ торговый путь и НЕ /info: обычный GET на stats-data, весового бюджета
// HL-пула не касается (см. инцидент голодания пула 19.07). Fail-soft во всём.
//
...
//
// 🚨 Возвращён 01.09.2026 суженным. Полный сбор сняли 28.08 при чистке
// лаборатории: вопрос «есть ли персистентность в децилях» был закрыт, а 18 МБ
// снимков копились под уже полученный ответ. Не заметили, что от того же файла
// кормится ЖИВОЙ предзаявленный тест «Гении Уолл-стрит» с датой решения
// 10.11.2026 — крон winners.mjs track с 28.08 падал на пустой директории,
// и форвардные данные не копились пять дней.
//
// Отсюда правило снимка: пишем ТОЛЬКО адреса из замороженного списка
// (выбранные + контроль + широкий), ~370 строк вместо 41k, десятки килобайт
// вместо мегабайт. Список заморожен 13.08 и меняться не может, поэтому такая
// фильтрация прибор не трогает: метрика та же (дельта allTime pnl/vlm по
// адресу), просто без строк, которые всё равно никто не читает.
