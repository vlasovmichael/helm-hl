...
const KEEP_SNAPSHOTS = 60; // ~14 месяцев недельного ритма
...
export async function captureSnapshot(now = new Date()) {
  ...
  const packed = rows.map(packRow).filter((a) => a[13] > 0); // allTime.vlm > 0
  ...
  pruneOld();
  return { ok: true, file, rows: packed.length, bytes: buf.length };
}
...
if (process.argv[1]?.endsWith("leaderboardSnapshot.mjs")) {
  ...
  console.log(r.ok ? `saved ${r.file} — ${r.rows} rows, ${(r.bytes / 1e6).toFixed(2)} MB` : `FAILED: ${r.reason}`);
